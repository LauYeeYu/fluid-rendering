...
def pbf_neighbour_search():
    ...
    for p in position:
        ...
        if p_grid[0] > grid_cols or p_grid[1] > grid_rows:
            print("grid error")
        g_index = ti.atomic_add(num_particle_in_grid[int(p_grid[0]), int(p_grid[1])], 1)
        # ---ERROR CHECK---
        if g_index >= max_particle_in_grid:
            print("Grid overflows.")
        table_grid[int(p_grid[0]), int(p_grid[1]), g_index] = p
    # ---update neighbour---
    for p in position:
        pos = position[p]
        # p_grid = (pos[0] // grid_size, pos[1] // grid_size)
            ...
            for off_y in ti.static(range(-1, 2)):
                if 0 <= p_grid[0] + off_x < grid_cols:
                    if 0 <= p_grid[1] + off_y < grid_rows:
                        ...
                        for i in range(num_particle_in_grid[int(nb[0]), int(nb[1])]):
                            new_nb = table_grid[int(nb[0]), int(nb[1]), i]
